...
/*Class*/ function FlashCoupleClass(){
    var CoupleData = [
         { man: "谢霆峰", woman: "王菲", priority: 1, rate: 40, hot: 3 },
         { man: "谢霆峰", woman: "张柏芝", priority: 1, rate: 25, hot: 3 },
         { man: "陈冠希", woman: "张柏芝", priority: 1, rate: 30, hot: 3 },
         { man: "李亚鹏", woman: "周迅", priority: 1, rate: 20, hot: 3 }, ];
    ...
    // Events
    this.CoupleTagClickEventHandler = null;

    this.loadCoupleData = function(){
        return CoupleData;
    }

    // 展现最大化、最小化界面
    this.showCoupleTags = function(prnNode, cpData, bMini){
        // remove all child-nodes
        while (prnNode.firstChild){
            prnNode.removeChild(prnNode.firstChild);
        }
        ...
        {
            // 创建HTML元素
            tagCouple = document.createElement("div");
            spanMan = document.createElement("span");
            spanWom = document.createElement("span");
            tagCouple.appendChild(spanMan);
            tagCouple.appendChild(spanWom);
            prnNode.appendChild(tagCouple);
            // 填写新结点的文本内容
            spanMan.innerHTML = cpData[i].man;
            spanWom.innerHTML = cpData[i].woman;

            if (bMini == null || bMini == false){
                // 调整新结点的大小、位置
                resizeCoupleTag(tagCouple, cpData[i].rate);
                relocateCoupleTag(prnNode, tagCouple);
            }
            spanMan.onclick = onclick_FlashCoupleTag;
            spanMan.onmouseover = onmouseover_Tag;
            spanMan.Context = this; // 把当前实例的引用保存到元素
            spanWom.onclick = onclick_FlashCoupleTag;
            spanWom.onmouseover = onmouseover_Tag;
            spanWom.Context = this; // 把当前实例的引用保存到元素
        }

        prnNode.style.display = "block";
    }

    this.closeCoupleTags = function(prnNode){
        // remove all child-nodes
        while (prnNode.firstChild){
            prnNode.removeChild(prnNode.firstChild);
        }
        prnNode.style.display = "none";
    }

    function resizeCoupleTag(tag, rate){
        tag.style.fontSize = rate/20 + "em";
    }

    function relocateCoupleTag(container, tag){
        div_width = container.clientWidth;
        tag_width = tag.clientWidth;
        tag_marginLeft = (div_width - tag_width)/2;
        tag_marginLeft = Math.random() * tag_marginLeft;
        tag.style.marginLeft = tag_marginLeft + "px";
    }

    function refillTagBkColor(tag, hot){
        rgb = getTagColor(hot);
        tag.style.backgroundColor = rgb;
    }

    // Call onclick event handler
    function onclick_FlashCoupleTag(e){
        var manName;
        var wonName;
        if (this.nextSibling == null){
            manName = this.previousSibling.innerHTML;
            wonName = this.innerHTML;
        }
        else{
            manName = this.innerHTML;
            wonName = this.nextSibling.innerHTML;
        }
        
        if (this.Context.CoupleTagClickEventHandler != null){
            this.Context.CoupleTagClickEventHandler(e, manName, wonName);
        }
    }

    function onmouseover_Tag(e){
